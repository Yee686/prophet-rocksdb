...
inline uint64_t FNVHash64(uint64_t val) {
  ...
  // FNVHash 1a 版本的核心：先异或，再乘
  // 使用展开的逻辑，减少循环分支
  hashval = (hashval ^ (val & 0xff)) * kFNVPrime64;
  hashval = (hashval ^ ((val >> 8) & 0xff)) * kFNVPrime64;
  hashval = (hashval ^ ((val >> 16) & 0xff)) * kFNVPrime64;
  hashval = (hashval ^ ((val >> 24) & 0xff)) * kFNVPrime64;
  hashval = (hashval ^ ((val >> 32) & 0xff)) * kFNVPrime64;
  hashval = (hashval ^ ((val >> 40) & 0xff)) * kFNVPrime64;
  hashval = (hashval ^ ((val >> 48) & 0xff)) * kFNVPrime64;
  hashval = (hashval ^ ((val >> 56) & 0xff)) * kFNVPrime64;
...
class ZipfianGenerator : public Generator<uint64_t> {
 public:
  ...
  ZipfianGenerator(uint64_t min, uint64_t max,
      ...
      : ZipfianGenerator(min, max, zipfian_const,
                         Zeta(max - min + 1, zipfian_const)) {}
...
class RandomByteGenerator : public Generator<char> {
 public:
  ...
  char Last() { return buf_[(off_ - 1 + 6) % 6]; }
...
class UniformGenerator : public Generator<uint64_t> {
 public:
  UniformGenerator(uint64_t min, uint64_t max)
      : generator_(std::random_device{}()),  // 使用硬件随机数做种子
        dist_(min, max) {
    Next();
  }
...
// public:
//   RandomGenerator(int min, int max): generator_(std::random_device{}()), int_dist_(min, max), real_dist_(min, max), last_(min) {}
//   ~RandomGenerator() {}
//   uint64_t Next() {
//     last_ = int_dist_(generator_);
//     return last_;
//   }

//   double NextReal() {
//     last_ = real_dist_(generator_);
//     return last_;
//   }
...
// private:
//   std::mt19937_64 generator_;
//   std::uniform_int_distribution<uint64_t> int_dist_;
...
//       abort();
//       throw new std::runtime_error("Too many unacknowledged insertion
//       keys.");
//     }
...
//   explicit ZipfianGenerator(uint64_t s, AcknowledgedCounterGenerator *keyseq,
//   uint64_t ub, uint64_t lb=0,  double factor=1.0)
//     : Generator(s),
...
//     return zipf_distribution<uint64_t, double>(range,
//     zipf_factor_)(generator_);
//   }
// };

// class ScrambledZipfianGenerator: public ZipfianGenerator {
//  public:
//   explicit ScrambledZipfianGenerator(uint64_t s, AcknowledgedCounterGenerator
//   *keyseq, uint64_t u, uint64_t l=0, double factor=1.0)
...
//   public:
//     explicit UnixEpochTimestampGenerator(long interval, long timeUnits, long
//     startTimestamp)
//       : Generator(0),
//         interval_(interval),
...
//  public:
//   explicit UniformGenerator(uint64_t s, AcknowledgedCounterGenerator *keyseq,
//   uint64_t u, uint64_t l=0)
//     : Generator(s),
//       trx_insert_keyseq_(keyseq),
//       lb_(l),
//       ub_(u) {};
...
//       ub_ = trx_insert_keyseq_->last_val();
//     }
//     return lb_ + std::uniform_int_distribution<uint64_t>(lb_,
//     ub_)(generator_);
//   }
// };
...
//   virtual uint64_t next_val() {
//     double val =
//     std::uniform_real_distribution<double>(0.0, 1.0)(generator_);
...
//  public:
//   SkewedLatestGenerator(uint64_t s, AcknowledgedCounterGenerator *keyseq,
//   uint64_t u, uint64_t l=0, double factor=1.0)
//     : Generator(s),
//       trx_insert_keyseq_(keyseq),
