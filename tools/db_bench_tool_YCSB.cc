...
// DEFINE_bool(enable_hotness_tracking,
//             ROCKSDB_NAMESPACE::Options().enable_hotness_tracking,
//             "Enable hotness tracking for DBImpl, "
//             "which will track the hotness of each key");
...
DEFINE_double(zipfian_const, 0.99, "Zipfian distribution constant (theta)");
...
DEFINE_double(
    deletion_ratio, 0.0,
    "Ratio of delete operation in delete benchmark [0.0 - 1.0]");
...
class Stats {
 private:
  ...
  std::vector<uint32_t> read_lat;
  std::vector<uint32_t> update_lat;
 ...
 public:
  ...
  // uint64_t op_count_[6]{0};
  // std::atomic<uint64_t> latency_sum_[5]{0};
  // uint64_t latency_sum_[6]{0};
  // uint64_t latency_min_[DBOperation::MAXOPTYPE];
  // uint64_t latency_max_[DBOperation::MAXOPTYPE];
  // std::atomic<uint64_t> op_bytes_[5]{0};
  // uint64_t op_bytes_[6]{0};

  std::vector<uint64_t> op_count_;
  std::vector<uint64_t> latency_sum_;
  std::vector<uint64_t> op_bytes_;

  Stats() : clock_(FLAGS_env->GetSystemClock().get()) {
    op_count_.assign(DBOperation::MAXOPTYPE, 0);
    latency_sum_.assign(DBOperation::MAXOPTYPE, 0);
    op_bytes_.assign(DBOperation::MAXOPTYPE, 0);
    Start(-1);
  }
  ...
  void Merge(const Stats& other) {
    ...
    for (uint64_t i = 0; i < DBOperation::MAXOPTYPE; i++) {
      ...
      if (i == DBOperation::READ) {
        read_lat.insert(read_lat.end(), other.read_lat.begin(), other.read_lat.end());
      } else if (i == DBOperation::UPDATE) {
        update_lat.insert(update_lat.end(), other.update_lat.begin(), other.update_lat.end());
      }
    }
  ...
  void ReportYCSB(enum DBOperation op_type, uint64_t latency, uint64_t bytes) {
    ...
    if (op_type == DBOperation::READ) {
      read_lat.emplace_back(latency);
    } else if (op_type == DBOperation::UPDATE) {
      update_lat.emplace_back(latency);
    }

    // if (op_type == DBOperation::UPDATE && true) {
    //   fprintf(stdout, "%lu us\n", latency);
    // }
  }
  ...
  void Report(const Slice& name) {
    ...
    if (is_ycsb_) {
      ...
      for (uint64_t i = 0; i < DBOperation::MAXOPTYPE; i++) {
        ...
        if (i == DBOperation::READ) {
          if (read_lat.size() > 0) {
            fprintf(stderr, "Computing READ statistics for %zu samples...\n", read_lat.size());
            fflush(stderr);
            
            mean = (double)lat_sum / cnt;
            std::vector<double> diff(cnt);
            std::transform(read_lat.begin(), read_lat.end(), diff.begin(), [mean](double x) { return x - mean; });
            sq_sum = std::inner_product(diff.begin(), diff.end(), diff.begin(), 0.0);
            stdev = std::sqrt(sq_sum / cnt);
            min_value = *std::min_element(read_lat.begin(), read_lat.end());
            max_value = *std::max_element(read_lat.begin(), read_lat.end());

            // 优化：只排序一次，然后直接取百分位数
            fprintf(stderr, "Sorting latency data for percentile calculation...\n");
            fflush(stderr);
            std::sort(read_lat.begin(), read_lat.end());
            
            size_t size = read_lat.size();
            p99 = read_lat[static_cast<size_t>(0.99 * size)];
            p99_9 = read_lat[static_cast<size_t>(0.999 * size)];
            p99_99 = read_lat[static_cast<size_t>(0.9999 * size)];
            p99_999 = read_lat[static_cast<size_t>(0.99999 * size)];
            
            fprintf(stdout,"************************************************\n");
            fprintf(stdout, "read percentile\n");
            fprintf(stdout, "99th : %u us, 99.9th : %u us, 99.99th : %u, 99.999th : %u\n", p99, p99_9, p99_99, p99_999);
            fflush(stdout);
          }
        } else if (i == DBOperation::UPDATE) {
          if (update_lat.size() > 0) {
            fprintf(stderr, "Computing UPDATE statistics for %zu samples...\n", update_lat.size());
            fflush(stderr);
            
            mean = (double)lat_sum / cnt;
            std::vector<double> diff(cnt);
            std::transform(update_lat.begin(), update_lat.end(), diff.begin(), [mean](double x) { return x - mean; });
            sq_sum = std::inner_product(diff.begin(), diff.end(), diff.begin(), 0.0);
            stdev = std::sqrt(sq_sum / cnt);
            min_value = *std::min_element(update_lat.begin(), update_lat.end());
            max_value = *std::max_element(update_lat.begin(), update_lat.end());
            ...
            size_t size = update_lat.size();
            p99 = update_lat[static_cast<size_t>(0.99 * size)];
            p99_9 = update_lat[static_cast<size_t>(0.999 * size)];
            p99_99 = update_lat[static_cast<size_t>(0.9999 * size)];
            p99_999 = update_lat[static_cast<size_t>(0.99999 * size)];
            
            fprintf(stdout,"************************************************\n");
            fprintf(stdout, "update percentile\n");
            fprintf(stdout, "99th : %u, 99.9th : %u, 99.99th : %u, 99.999th : %u\n", p99, p99_9, p99_99, p99_999);
            fflush(stdout);
          }
        }
        fprintf(stdout, "bytes sum : %lu lat sum : %lu\n", bytes_sum, lat_sum);
        fprintf(stdout, "op : %ld cnt : %ld Avg Lat : %lf micros, %lf ops/s stdev : %lf\n", i, cnt, ((cnt > 0) ?
                          mean : 0),  static_cast<double>(cnt) / elapsed, stdev);
...
class Benchmark {
 private:
  ...
  int64_t num_;
  int64_t num_op_; // YCSB-A/B/C/....  总操作数
  int64_t num_record_; // YCSBLOAD 总记录数 
  ...
  // YCSB
  CounterGenerator *insert_key_sequence_; // counter generator -> load operation
  Generator<uint64_t> *key_chooser_;
  Generator<uint64_t> *scan_length_; // uniform generator
  DiscreteGenerator<util::DBOperation> operation_chooser_;  // 操作选择器, 通过配置的概率选择操作
  ...
  bool write_all_fields_; // default : true
...
// Current the following isn't equivalent to OS_LINUX.
  ...
  std::string BuildKeyName(uint64_t key_num) {
    ...
    // 使用 thread_local 缓存字符串缓冲区
    thread_local std::string cache;
    cache.clear();
    cache.reserve(4 + zero_padding_ + 20);
    cache = "user";
    
    char num_buf[32];
    int num_len = snprintf(num_buf, sizeof(num_buf), "%" PRIu64, key_num);
    
    int fill = std::max(0, zero_padding_ - num_len);
    cache.append(fill, '0');
    cache.append(num_buf, num_len);
    
    return cache; // 返回时会触发移动构造，避免拷贝
  }
  ...
  void BuildValues(std::vector<Field>& values) {
    // 预分配空间，避免多次重分配
    values.reserve(field_count_);
    
    // 复用 RandomByteGenerator
    RandomByteGenerator byte_generator;
    
    // 预计算 field_prefix_ 的长度，用于预分配
    size_t prefix_len = field_prefix_.size();
    
    for (int i = 0; i < field_count_; i++) {
      // 使用 emplace_back 避免默认构造 + 赋值
      values.emplace_back();
      Field& field = values.back();
      
      // 预分配 field.name 的空间
      // "field" (5) + 最多10位数字
      field.name.reserve(prefix_len + 10);
      field.name = field_prefix_;
      
      // 使用更快的整数转字符串（避免 std::to_string）
      char num_buf[16];
      int num_len = snprintf(num_buf, sizeof(num_buf), "%d", i);
      field.name.append(num_buf, num_len);
      
      uint64_t len = field_len_generator_->Next();
      field.value.reserve(len);
      
      // 直接写入，避免 lambda 开销
      field.value.resize(len);
      for (uint64_t j = 0; j < len; ++j) {
        field.value[j] = byte_generator.Next();
      }
    }
  }
  ...
  void BuildSingleValue(std::vector<Field>& values) {
    ...
    // 复用 NextFieldName 的优化版本
    field.name.reserve(field_prefix_.size() + 10);
    field.name = field_prefix_;
    char num_buf[16];
    int num_len = snprintf(num_buf, sizeof(num_buf), "%" PRIu64, 
                           field_chooser_->Next());
    field.name.append(num_buf, num_len);
    ...
    // 复用 RandomByteGenerator（如果可能，使用成员变量）
    static thread_local RandomByteGenerator byte_generator;
    field.value.resize(len);
    for (uint64_t i = 0; i < len; ++i) {
      field.value[i] = byte_generator.Next();
    }
  }
  ...
  std::string NextFieldName() {
    thread_local std::string cache;
    cache.clear();
    cache.reserve(field_prefix_.size() + 10);
    cache = field_prefix_;
 ...
 public:
  ...
  void Run() {
    std::cout << "This is DB_Bench with RANDOM/YCSB" << std::endl;
    if (!SanityCheck()) {
      ErrorExit();
    }
    Open(&open_options_);
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      std::cout << "num_op(总操作数) : " << num_op_ << ",  num_record(总记录数) : " << num_record_ << std::endl;
      deletion_ratio = (FLAGS_deletion_ratio >= 0 && FLAGS_deletion_ratio <= 1) ? FLAGS_deletion_ratio : 0.2;
      reads_ = (FLAGS_reads < 0 ? FLAGS_num : FLAGS_reads);
      writes_ = (FLAGS_writes < 0 ? FLAGS_num : FLAGS_writes);
      ...
      insert_key_sequence_ = new CounterGenerator(0);
      ...
      std::cout << "benchmark is [" << name << "] " << std::endl;
      ...
      if (name == "fillseqdeterministic" ||
        ...
        fresh_db = false;
        key_chooser_ = new ScrambledZipfianGenerator(0, num_record_- 1, FLAGS_zipfian_const);
        ...
        key_chooser_ = new ScrambledZipfianGenerator(0, num_record_ + new_keys- 1, FLAGS_zipfian_const);
        ...
        method = &Benchmark::YcsbTxn;
      } else if (name == "deletemixed") {
        fresh_db = false;
        key_chooser_ = new ScrambledZipfianGenerator(0, num_record_- 1, FLAGS_zipfian_const);
        operation_chooser_.AddValue(DBOperation::DELETE, deletion_ratio);
        operation_chooser_.AddValue(DBOperation::READ, (1 - deletion_ratio)*0.8);
        operation_chooser_.AddValue(DBOperation::INSERT, (1 - deletion_ratio)*0.2);
        method = &Benchmark::YcsbTxn;
      } else if (name == "deletewrite") {
        fresh_db = false;
        key_chooser_ = new ScrambledZipfianGenerator(0, num_record_- 1, FLAGS_zipfian_const);
        operation_chooser_.AddValue(DBOperation::DELETE, deletion_ratio);
        operation_chooser_.AddValue(DBOperation::INSERT, 1 - deletion_ratio);
        method = &Benchmark::YcsbTxn;
      } else if (name == "deleteread") {
        fresh_db = false;
        key_chooser_ = new ScrambledZipfianGenerator(0, num_record_- 1, FLAGS_zipfian_const);
        operation_chooser_.AddValue(DBOperation::DELETE, deletion_ratio);
        operation_chooser_.AddValue(DBOperation::READ, 1 - deletion_ratio);
        method = &Benchmark::YcsbTxn;
      } else if (name == "deleteheavy") {
        deletes_ = static_cast<int64_t>(FLAGS_num * deletion_ratio);
        method = &Benchmark::DeleteHeavy;
      ...
      if (method != nullptr) {
        ...
        for (int i = 0; i < num_repeat; i++) {
          Stats stats;
          if (name == "YCSBLOAD" || name == "fillrandom") {
            num_threads = 1;
    ...
    if (FLAGS_statistics) {
      fflush(stdout);  // 确保之前的输出都被刷新
      fprintf(stdout, "STATISTICS:\n%s\n", dbstats->ToString().c_str());
      fflush(stdout);  // 立即刷新 STATISTICS 输出
    }
    if (FLAGS_simcache_size >= 0) {
      fprintf(
 ...
 private:
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    // options.enable_hotness_tracking = FLAGS_enable_hotness_tracking;
  ...
  void DoYCSB(ThreadState *thread, YCSB_MODE mode) {
    if (mode == YCSB_LOAD) {
      ...
      Duration duration(0, num_record_);
      Slice value;
      RandomGenerator gen;
      
      while (!duration.Done(1)) {
        std::string key = BuildKeyName(insert_key_sequence_->Next());
        ...
        // fprintf(stdout, "YCSB LOAD key : %ld value : %ld\n", key.size(), data.size());
        rocksdb::Status s = db_.db->Put(write_options_, key, data);
        // rocksdb::Status s = db_.db->Put(write_options_, key, val);

        if (!s.ok()) {
          std::string err = "ycsb put error: " + s.ToString();
          fprintf(stdout, "%s\n", err.c_str());
          thread->stats.AddMessage(err);
          break;  // 终止本次 YCS BLOAD，但仍然输出统计信息
        }
      ...
      while (!duration.Done(1)) {
        ...
        if (op == DBOperation::READ) {
          ...
          /*if (!read_all_fields_) {
            ...
            rocksdb::Iterator *db_iter = db_.db->NewIterator(rocksdb::ReadOptions());
            db_iter->Seek(key);
          ...
          thread->stats.AddBytes(bytes);
        } else if (op == DBOperation::DELETE) {
            uint64_t key_num = NextTxnKeyNum();
            std::string key = BuildKeyName(key_num);
            uint64_t start = FLAGS_env->NowMicros();
            
            rocksdb::Status s = db_.db->Delete(write_options_, key);
            
            if (!s.ok()) {
                fprintf(stderr, "ycsb delete error\n");
                abort();
            }
            
            uint64_t elapsed = FLAGS_env->NowMicros() - start;
            thread->stats.ReportYCSB(op, elapsed, key.size());
    ...
    while ((num_per_key_gen != 0) && !duration.Done(entries_per_batch_)) {
      ...
      for (int64_t j = 0; j < entries_per_batch_; j++) {
        ...
        if (use_blob_db_) {
          ...
          // fprintf(stdout, "key size : %ld, value size : %ld\n", key.size(), val.size());
          batch.Put(key, val);
        } else {
...
// optimization, so skip the check of the max level.
  ...
  void DoDelete(ThreadState* thread, bool seq) {
    ...
    while (!duration.Done(entries_per_batch_)) {
      ...
      thread->stats.AddBytes(batch.GetDataSize());
  ...
  //随机删除num * deletion_ratio个键
  void DeleteHeavy(ThreadState* thread) {
    // DoWrite(thread, SEQUENTIAL); // 无需再写, 直接先使用fillseq填充数据再删除
