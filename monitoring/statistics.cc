...
const int kTmpStrBufferSize = 1000;
  ...
  for (const auto& h : HistogramsNameMap) {
    ...
    int ret =
        snprintf(buffer, kTmpStrBufferSize,
                 "%s [P50 : %f] [P95 : %f] [P99 : %f] [P99.9 : %f] [P99.99 : %f] [COUNT : %" PRIu64
                 "] [SUM : %" PRIu64 "] \n",
                 h.second.c_str(), hData.median, hData.percentile95,
                 hData.percentile99, hData.percentile999, hData.percentile9999, hData.count, hData.sum);
